      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-5 space-y-8">
            ...
            <div className="bg-white border border-[#8A1C36]/15 p-8 space-y-6 shadow-sm">
              <h3 className="font-serif text-xs font-bold tracking-[0.25em] text-brand-gold uppercase border-b border-brand-gold/15 pb-3">
                DIRECCIÓN ARTÍSTICA
              </h3>
              <div>
                <p className="font-serif text-lg font-bold text-gray-800">María Lucía Roca Gamarra</p>
              ...
              <div>
                <p className="font-serif text-lg font-bold text-gray-800">Julián Enríquez</p>
                <p className="text-xs text-gray-500 font-light mt-1">Dirección Musical · Flauta · Adaptación de Arreglos</p>
              </div>
            </div>

            <div className="bg-white border border-[#8A1C36]/15 p-8 space-y-4 text-xs text-gray-650 shadow-sm">
              <h3 className="font-serif font-bold tracking-[0.25em] text-gray-800 uppercase mb-4">
          ...
          <div className="lg:col-span-7">
            <div className="bg-white border border-[#8A1C36]/15 p-8 sm:p-10 shadow-sm">
              <h3 className="font-serif text-sm font-bold tracking-[0.2em] uppercase text-gray-800 mb-6">
                ENVÍANOS UN MENSAJE
              </h3>
              ...
              {isSubmitted ? (
                <div className="py-12 text-center">
                  ...
                  <h4 className="font-serif text-lg text-gray-850 mb-2">¡Mensaje Recibido!</h4>
                  <p className="text-xs text-gray-655 mb-6">Gracias por contactarte con CINEFONÍA Nights. Te responderemos a la brevedad.</p>
                  <button
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 text-xs font-sans">
                  <div>
                    <label className="block text-[10px] uppercase font-bold text-gray-600 mb-1">Nombre Completo</label>
                    <input
                      type="text"
                      {...register("name")}
                      placeholder="Tu nombre"
                      className="w-full bg-[#FAF9F5] border border-gray-200 focus:border-brand-gold px-4 py-3 text-gray-800 placeholder-gray-400 focus:outline-none transition-colors"
                    />
                    {errors.name && <p className="text-[10px] text-red-500 mt-1">{errors.name.message}</p>}
                  </div>

                  <div>
                    <label className="block text-[10px] uppercase font-bold text-gray-600 mb-1">Correo Electrónico</label>
                    <input
                      type="email"
                      {...register("email")}
                  ...
                  <div>
                    <label className="block text-[10px] uppercase font-bold text-gray-600 mb-1">Asunto</label>
                    <input
                      type="text"
                      {...register("subject")}
                  ...
                  <div>
                    <label className="block text-[10px] uppercase font-bold text-gray-600 mb-1">Mensaje</label>
                    <textarea
                      rows={5}
                      {...register("message")}
                      placeholder="Escribe tu mensaje aquí..."
                      className="w-full bg-[#FAF9F5] border border-gray-200 focus:border-brand-gold px-4 py-3 text-gray-800 placeholder-gray-400 focus:outline-none transition-colors resize-none"
                    />
                    {errors.message && <p className="text-[10px] text-red-500 mt-1">{errors.message.message}</p>}
                  </div>

                  {submitError && (
                    <div className="p-3 border border-red-505/30 bg-red-500/5 text-red-500 text-xs font-medium rounded-none">
                      {submitError}
                    </div>
                  )}
                  ...
                  <button
                    type="submit"
                    disabled={isLoading}
                    className="w-full bg-[#8A1C36] hover:bg-[#5E0B1F] text-white font-bold tracking-[0.2em] uppercase py-3.5 px-6 transition-colors flex items-center justify-center space-x-2 cursor-pointer"
                  >
